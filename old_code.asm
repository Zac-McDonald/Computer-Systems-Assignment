...
; Validation for draw pixel - removed because I like abusing y=0 x=i do draw pixels using the index
	and r0, $FFFF 				; Ensure that the colour is only 16 bits

	cmp r1, #-1					; Check if x <= -1
	POPle { pc }				; If it is, return because it's an offscreen point
	cmp r1, SCREEN_WIDTH		; If it is not, check if x >= SCREEN_WIDTH
	POPge { pc }				; If it is, return because it's an offscreen point

	cmp r1, #-1					; Same checking for y
	POPle { pc }
	cmp r1, SCREEN_WIDTH
	POPge { pc }